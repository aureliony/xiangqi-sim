...
class Robotiq2F85:
    ...
    def __init__(self, robot, tool, position):
        ...
        print("Creating new constraint for initial attachment")
        self.constraint_id = pybullet.createConstraint(
            self.robot, tool, self.body, 0, jointType=pybullet.JOINT_FIXED, jointAxis=[0, 0, 0],
    ...
    def attach(self, tool_position):
        print("Attaching gripper...")
        """Reattach the gripper to the robot tool after path planning."""
        # Reconnect gripper base to robot tool at a specified position.
        self.constraint_id = pybullet.createConstraint(
            self.robot, 
            self.tool, 
            self.body, 
            ...
            jointAxis=[0, 0, 0], 
            parentFramePosition=tool_position, 
    ...
    def detach(self):
        ...
        """Detach the gripper from the robot tool."""
        if hasattr(self, 'constraint_id') and self.constraint_id is not None:
            try:
                print(f"Removing constraint with ID: {self.constraint_id}")
                pybullet.removeConstraint(self.constraint_id)
                self.constraint_id = None
                self.activated = False
            except Exception as e:
                print(f"Failed to remove constraint: {e}")
        else:
            print("[DEBUG] No valid constraint to remove.")
...
# Add RRTPathPlanner class
class RRTPathPlanner:
    def __init__(self, robot_id, joint_ids, step_size=0.1, max_iterations=10000, goal_tolerance=0.005):
        ...
        self.bounds = [
            [-np.pi, np.pi],  # Joint 0 bounds
            [-np.pi, np.pi],  # Joint 1 bounds
            [-np.pi, np.pi],  # Joint 2 bounds
            [-np.pi, np.pi],  # Joint 3 bounds
            [-np.pi, np.pi],  # Joint 4 bounds
            [-np.pi, np.pi],  # Joint 5 bounds
        ]

    def sample_random_configuration(self):
        """Samples a random joint configuration within the bounds."""
    ...
    def interpolate(self, config1, config2, step):
        """Interpolates between two configurations by a given step size."""
        direction = np.array(config2) - np.array(config1)
        distance = np.linalg.norm(direction)
        if distance < step:
            return config2
        direction /= distance
        return np.array(config1) + direction * step

    def is_collision_free(self, config):
        """Checks if the robot is in a collision-free state for a given joint configuration."""
        for joint_id, joint_angle in zip(self.joint_ids, config):
            pybullet.resetJointState(self.robot_id, joint_id, joint_angle)
        
        # Assuming we can check collisions using PyBullet
        points = pybullet.getContactPoints(bodyA=self.robot_id)
        return len(points) == 0

    def plan_path(self, start, goal, return_iterations=False):
        """Plans a path using RRT from start to goal configuration."""
        ...
        for _ in range(self.max_iterations):
            ...
            # Sample a random configuration or the goal with a probability of 10%
            ...
            nearest_config = min(tree.keys(), key=lambda node: self.distance(node, random_config))
            new_config = self.interpolate(nearest_config, random_config, self.step_size)

            # Add the new node if it's collision-free
            if self.is_collision_free(new_config):
                tree[tuple(new_config)] = nearest_config
                if self.distance(new_config, goal) < self.goal_tolerance:
                    path = self.reconstruct_path(tree, tuple(new_config))
    ...
    def reconstruct_path(self, tree, end_config):
        """Reconstructs the path from the start to the goal configuration."""
        path = [end_config]
        current = end_config
    ...
    def smooth_path(self, path):
        """Smooths the given path by removing unnecessary waypoints."""
        smoothed_path = [path[0]]
        for i in range(2, len(path)):
            if not self.is_collision_free(self.interpolate(smoothed_path[-1], path[i], self.step_size)):
